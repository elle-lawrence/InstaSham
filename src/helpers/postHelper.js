...
const getAllPosts = (userId) => new Promise((resolve, reject) => {
  axios
    .get(`${dbURL}/posts.json?orderBy="userId"&equalTo="${userId}"`)
...
const createPost = (postObject) => new Promise((resolve, reject) => {
  axios
    .post(`${dbURL}/posts.json`, postObject)
    .then((response) => {
...
const deletePost = (postId) => new Promise((resolve) => {
  ...
  axios
    .delete(`${dbURL}/posts/${postId}.json`)
    .then(() => getAllPosts().then(resolve({ postId })));
});

export {
  deletePost, getAllPosts, getSinglePost, createPost,
};
